...
public class FormattingConversionService extends GenericConversionService
	...
	private static class ParserConverter implements GenericConverter {
		...
		public Object convert(@Nullable Object source, TypeDescriptor sourceType, TypeDescriptor targetType) {
			...
			try {
				...
				if (text.matches("[0-9]{13}")) {
					return convert(textTemp, targetType);
				}
		 ...
		 * @param text 13位长度
		 */
		private Object convert(String text, TypeDescriptor targetType) {
			...
			if (objectType.equals(LocalDateTime.class)) {
				return LocalDateTime.ofInstant(instant, zoneId);
			}
			if (objectType.equals(LocalDate.class)) {
				return LocalDate.ofInstant(instant, zoneId);
			}
			if (objectType.equals(Date.class)) {
				return Date.from(instant);
			}
			throw new UnsupportedOperationException("暂不支持转换的日期类型：" + objectType.getName());
		}
